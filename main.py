    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            symbol_dict = {
                ...
                "S1": None,"S2": None,"S3": None,"AllowedDiff":None,"ActualDiff":None,"Trade":None,"TargetExecuted":None
            }
...
# Run only if the current time has crossed the next scheduled fetch time
            if time.time() >= last_fetch_start + timeframe:
                result_dict[symbol]["last_ohlc_fetch_start_time"] = iteration_start_time  # ✅ mark start time of fetch
...
if __name__ == "__main__":
    ...
    if xts_marketdata:
        while True:
            ...
            main_strategy()
            # Align to next clean RUN_INTERVAL_SECONDS block
            # seconds_since_hour = now.minute * 60 + now.second
            # wait = RUN_INTERVAL_SECONDS - (seconds_since_hour % RUN_INTERVAL_SECONDS)
